...
import com.fengwo.module_chat.widgets.chat_new.AudioPrintView;
...
//    private String mId;
    public VoicePlayHelper(final Context context, String fromId) {
        mContext = context;
        this.mFromId = fromId;
        mVoicePlayer = new VoicePlayerEx(context, true, (observable, o) -> {
            // 播放结束提示音
        ...
        try {
            ...
            if (voiceFile.exists()) {
                try {
                    ...
                    L.e(TAG, mContext.getString(R.string.chat_playvoice_play_faild));
...
//                }
            ...
            L.e(TAG, "此消息不符合语音消息格式！");
        }
        return sucess;
    }

    @SuppressLint("StaticFieldLeak")
    public void loadDownVoiceAndPlay(String fileUrl, String dir) {
...
//                    entity.getDownloadStatus().setStatus(ChatMsgEntity.DownloadStatus.PROCESS_OK);
                    ...
                    L.e(TAG, mContext.getString(R.string.chat_playvoice_play_faild3));
            ...
            protected void onPostExecute_onException(Exception exception) {
                ...
                L.e(TAG, mContext.getString(R.string.chat_playvoice_play_faild4));
