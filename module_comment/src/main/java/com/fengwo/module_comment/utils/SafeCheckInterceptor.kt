...
class SafeCheckInterceptor : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (TextUtils.equals(request.method, "POST") || TextUtils.equals(request.method, "PUT") || TextUtils.equals(request.method, "DELETE")) {
            var body = ""
            if (reqBody != null) {
                body = getRequestBody(reqBody)
                KLog.i("gkh", "body = $body")
            }
        ...
        var version = AppUtils.getVersionName(BaseApplication.mApp)
        if (version == null) {
            version = ""
        }
        requestBuilder
                .addHeader("X-MAC", DeviceUtils.getMacAddress())
                .addHeader("X-VERSION", version)
                .addHeader("X-SOURCE-ID", DeviceUtils.getAndroidID())
                .addHeader("X-PHONE-TYPE", DeviceUtils.getModel())
                .addHeader("X-SYSTEM-VERSION", DeviceUtils.getSDKVersionName())
