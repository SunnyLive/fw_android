...
public class DanmakuView extends View implements IDanmakuView, IDanmakuViewController {
    ...
    private synchronized void stopDraw() {
        ...
        try {
            //反射解决内存泄漏
            Class mClass = DrawTask.class;
            Field mTaskListenerField = mClass.getDeclaredField("mTaskListener");
            mTaskListenerField.setAccessible(true);
            mTaskListenerField.set(handler.drawTask, null);
        } catch (Exception e) {
            e.fillInStackTrace();
        }
    ...
    private Runnable mResumeRunnable = new Runnable() {
        @Override
        public void run() {
            KLog.e("danmu", "danmu" + mResumeTryCount);
