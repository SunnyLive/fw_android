...
public class HuafenProgressBar extends View {
    ...
    private float mBubbleTriangleHeight = 10;//气泡底部小三角高度
    private float mBubbleRectRound = 10;//气泡的圆角
    private String mProgressStr = "0";//显示进度的字符串
    private float mTextSize = 20;//进度条文字大小
    private Paint.FontMetricsInt mFontMetricsInt;
    private float mProgressStrMargin = 20;//气泡的边距
    private int max = 100;
    private boolean showText = true;

    private Context mContext;

    public HuafenProgressBar(Context context) {
        this(context, null);
    }

    public HuafenProgressBar(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HuafenProgressBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        mProgressHeight = mContext.getResources().getDimension(R.dimen.dp_5);
        ...
        mPaintProgressStr.setColor(mContext.getResources().getColor(R.color.colorAccent));
        mTextSize = mContext.getResources().getDimension(R.dimen.sp_14);
        ...
        mColorProgressBg = Color.parseColor("#66FFFFFF");
        mColorProgress = Color.parseColor("#FFEE7F");
        mPaintBubble.setColor(getResources().getColor(R.color.white));//设置气泡的颜色

        mFontMetricsInt = mPaintProgressStr.getFontMetricsInt();


        mProgressStrMargin = mContext.getResources().getDimension(R.dimen.dp_12);
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mPathSrc.moveTo(0 + params.leftMargin, h - mProgressHeight * 2);
        mPathSrc.lineTo(w - params.rightMargin, h - mProgressHeight * 2);//进度条位置在控件整体底部，且距离控件左边和右边各20像素
    ...
    public void setProgressWithAnim(float progress, boolean show) {
        showText = show;
        setProgressWithAnim(progress);
    }
}