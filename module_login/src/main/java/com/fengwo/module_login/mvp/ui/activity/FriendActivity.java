...
public class FriendActivity extends BaseListActivity<FansDto> {

    private AMapLocation location;
    private int type = 0;//0好友 1关注 2粉丝
    ...
    public Flowable setNetObservable() {
        ...
        return new RetrofitUtils().createApi(LoginApiService.class).getFriendList(p, location == null ? null : location.getLatitude() + "", location == null ? null : location.getLongitude() + "", 1);
    }
    ...
    public void bingViewHolder(BaseViewHolder helper, FansDto item, int position) {
        ...
        if (item.userState == 0) {//是否被封禁
            ...
            helper.setGone(R.id.iv_gif_root, item.liveStatus == 2 || item.wenboLiveStatus == 1);
            ...
            helper.getView(R.id.civ_head_root).setOnClickListener(v -> {
                if (item.liveStatus == 2) {
                    ...
                    if (FloatingView.getInstance().isShow()) {
                        ...
                        //    LivingRoomActivity.start(FriendActivity.this, list, 0, true);
                    }
                    ...
                    MineDetailActivity.startActivityWithUserId(FriendActivity.this, adapter.getData().get(position).id);
                }
            });
        }
        ...
        dialog.setNegativeButtonText("取消")
                ...
                .addDialogClickListener(new BaseDialogFragment.OnClickListener() {
                    ...
                    public void onConfirm() {
                        IntentRoomActivityUrils.setRoomActivity(list.get(0).channelId,list,0);
                //        LivingRoomActivity.start(FriendActivity.this, map, 0, true);
                    }
